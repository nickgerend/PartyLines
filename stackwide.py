...
for name, group in df_group: 
    ...
    for index, row in year.iterrows():
        if maxcheck == True:
            maxvalue = row['t_totalvotes']
            maxcheck = False
        if row['2016_rank'] != istate:
            xadd += ix
        row['x2'] = row['x'] + xadd + separation * row['2016_rank']
        row['x3'] = row['x'] * (maxvalue / (row['2016_votes']/100000)) + (maxvalue + separation) * row['2016_rank']
        data.append(row)
...
print(df_state)
df_state.to_csv(os.path.dirname(__file__) + '/state_test5.csv', encoding='utf-8', index=False)